...
Find the Codex auto-memory directory for this project. It's based on the working directory path:

1. Run `pwd` to get the current directory.
2. Construct the memory path: `~/.Codex/projects/` + the cwd with `/` replaced by `-` and leading `-` (e.g., `/Users/alice/Source/github.com/fleetdm/fleet` → `~/.Codex/projects/-Users-alice-Source-github-com-fleetdm-fleet/memory/`).
...
6. If they want you to scan prior sessions, look at the JSONL transcript files in the Codex project directory (the parent of the memory directory). Read recent ones (last 5-10), skimming for messages related to the workstream. These are large files, so read selectively — check the first few hundred lines of each to gauge relevance before reading more deeply.
